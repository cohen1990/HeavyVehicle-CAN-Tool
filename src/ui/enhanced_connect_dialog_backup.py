...
class EnhancedConnectDialog(QDialog):
    ...
    def accept(self):
        ...
        try:
            ...
            # 连接进度对话框
            progress_dialog = QProgressDialog("正在连接设备...", "取消", 0, len(self.connected_devices), self)
            progress_dialog.setWindowTitle("连接设备")
            ...
            for i, device_info in enumerate(self.connected_devices):
                ...
                # 更新进度
                progress_dialog.setLabelText(f"正在连接: {device_name}")
            ...
            # 关闭进度对话框
            progress_dialog.setValue(len(self.connected_devices))
            ...
            if connected_count > 0:
                ...
                QTimer.singleShot(800, lambda: super().accept())
            else:
                # 所有设备连接失败
                QMessageBox.warning(
                    self,
                    "连接失败",
                    f"所有设备连接失败 ({failed_count} 个设备)\n\n"
                    "可能原因：\n"
                    "1. 硬件未正确安装\n"
                    "2. 端口被占用\n"
                    "3. 驱动程序问题\n"
